@php
    $title = 'Tambah Kursus Baru - Super Admin';
@endphp
...
<x-app-layout :title="$title">
    <div style="max-width: 680px; margin: 0 auto; width: 100%;">
        ...
        <div style="margin-bottom: 24px;">
            ...
            <h1 style="font-size: 24px; font-weight: 900; color: #0f172a;">Buat Kursus Baru</h1>
            <p style="color: #64748b; font-size: 14px;">Tambah topik pemrograman baru dan tugaskan ke guru/mentor pengampu.</p>
        </div>
        ...
        <div class="card-3d" style="padding: 28px;">
            <form action="{{ route('admin.courses.store') }}" method="POST" style="display: flex; flex-direction: column; gap: 18px;">
                @csrf
                ...
                <div>
                    ...
                    <select name="mentor_id" required style="width: 100%; padding: 12px 16px; border: 2px solid #cbd5e1; border-radius: 14px; font-size: 14px; font-weight: 700; color: #0f172a; outline: none; background: #ffffff;">
                        @foreach ($mentors as $m)
                            <option value="{{ $m->id }}" {{ old('mentor_id') == $m->id ? 'selected' : '' }}>{{ $m->name }} ({{ $m->role }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Dasar Pemrograman JavaScript Interaktif" style="width: 100%; padding: 12px 16px; border: 2px solid #cbd5e1; border-radius: 14px; font-size: 14px; font-weight: 600; outline: none;">
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                    <div>
                        ...
                        <input type="text" name="category" value="{{ old('category') }}" required placeholder="Contoh: JavaScript Dasar" style="width: 100%; padding: 12px 16px; border: 2px solid #cbd5e1; border-radius: 14px; font-size: 14px; font-weight: 600; outline: none;">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Jelaskan apa yang akan dipelajari siswa dalam kursus ini..." style="width: 100%; padding: 12px 16px; border: 2px solid #cbd5e1; border-radius: 14px; font-size: 14px; font-weight: 600; outline: none;">{{ old('description') }}</textarea>
                </div>
                ...
                <div style="display: flex; align-items: center; gap: 10px; padding: 10px 0;">
                    ...
                    <label for="is_published" style="font-size: 14px; font-weight: 800; color: #0f172a; cursor: pointer;">Publikasikan kursus ini agar langsung bisa diakses siswa</label>
                </div>

                <div style="margin-top: 10px; display: flex; gap: 12px;">
                    <button type="submit" class="btn-3d btn-blue" style="flex: 1; padding: 14px;">
                        Simpan &amp; Lanjut Buat Modul
                    </button>
