@php
    ...
    $unitGradients = [
        ['bg' => 'linear-gradient(135deg, #1d4ed8, #0284c7)', 'shadow' => '#1e40af', 'badge' => '#bfdbfe'],
        ['bg' => 'linear-gradient(135deg, #4338ca, #7c3aed)', 'shadow' => '#3730a3', 'badge' => '#ddd6fe'],
        ['bg' => 'linear-gradient(135deg, #047857, #0d9488)', 'shadow' => '#065f46', 'badge' => '#a7f3d0'],
        ['bg' => 'linear-gradient(135deg, #b45309, #ea580c)', 'shadow' => '#9a3412', 'badge' => '#fed7aa'],
    ];
@endphp
...
<x-app-layout :title="$title">
    <style>
        ...
        .unit-roadmap-wrapper {
            ...
            padding: 36px 20px;
            margin-bottom: 40px;
            box-shadow: 0 8px 24px -6px rgba(37, 99, 235, 0.05), 0 4px 0 #e2e8f0;
        }
    ...
    <div class="content-container">
        ...
        <div style="width: 100%;">
            ...
            @foreach ($units as $unitIndex => $unit)
                @php
                    $theme = $unitGradients[$unitIndex % count($unitGradients)];
                @endphp

                <!-- Vibrant Unit Header Banner -->
                <div class="unit-banner" style="background: {{ $theme['bg'] }}; border-radius: 24px; padding: 24px; margin-bottom: 24px; box-shadow: 0 6px 0 {{ $theme['shadow'] }}, 0 12px 20px -8px rgba(0, 0, 0, 0.15); position: relative; overflow: hidden;">
                    <!-- Subtle background badge icon watermark -->
                    <div style="position: absolute; right: -15px; bottom: -20px; opacity: 0.12; color: #ffffff; pointer-events: none;">
                        <svg width="120" height="120" viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
                    </div>

                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; position: relative; z-index: 2;">
                        <div>
                            <div style="font-size: 11px; font-weight: 800; text-transform: uppercase; color: {{ $theme['badge'] }}; letter-spacing: 0.8px;">
                                {{ $unit['title'] }}
                            </div>
                        ...
                        <div style="width: 44px; height: 44px; background: rgba(255, 255, 255, 0.2); backdrop-filter: blur(8px); border-radius: 14px; display: flex; align-items: center; justify-content: center; color: #fff; flex-shrink: 0; border: 1px solid rgba(255, 255, 255, 0.3);">
                ...
                <!-- Styled Zigzag Roadmap Adventure Track -->
                <div class="unit-roadmap-wrapper">
                    <svg class="roadmap-svg-connector" id="svg-unit-{{ $unitIndex }}"></svg>

                    <div style="display: flex; flex-direction: column; align-items: center; gap: 40px; position: relative; z-index: 2;">
                        @php
                            $offsets = [0, 52, 0, -52];
                        @endphp
                                ...
                                <div class="node-btn-wrapper" data-completed="{{ $lesson['is_completed'] ? 'true' : 'false' }}" data-unlocked="{{ $lesson['is_unlocked'] ? 'true' : 'false' }}">
                                        ...
                                        <a href="{{ route('learn.lesson', $lesson['id']) }}" 
                                           style="text-decoration: none; position: relative; display: inline-block;">
                                            ...
                                            @if ($lesson['is_current'])
                                                ...
                                                <div style="position: absolute; top: -38px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #2563eb, #1d4ed8); color: #fff; font-size: 11px; font-weight: 900; text-transform: uppercase; padding: 6px 14px; border-radius: 12px; box-shadow: 0 4px 0 #1e40af; white-space: nowrap; z-index: 10;" class="animate-float">
                                            ...
                                            <!-- Circular Node Button with Vibrant Gradient -->
                                            <div class="node-circle btn-3d {{ $lesson['is_completed'] ? 'btn-green' : ($lesson['is_current'] ? 'btn-blue pulse-active-node' : 'btn-blue') }}"
                                                 style="{{ $lesson['is_completed'] ? 'background: linear-gradient(135deg, #10b981, #059669); box-shadow: 0 6px 0 #047857;' : 'background: linear-gradient(135deg, #2563eb, #3b82f6); box-shadow: 0 6px 0 #1e40af;' }}">
                                    ...
                                    @else
                                        ...
                                        <div class="node-circle"
                                             style="background: #e2e8f0; border: 2px solid #cbd5e1; box-shadow: 0 6px 0 #cbd5e1; color: #94a3b8; cursor: not-allowed;">
        ...
        <!-- Right Column: Vibrant Gamification HUD Widgets -->
        <div style="display: flex; flex-direction: column; gap: 20px; width: 100%;">
            
            <!-- Top Stats Multi-Color Card Grid -->
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px;">
                <!-- Streak Card -->
                <div class="card-3d" style="padding: 14px 10px; text-align: center; background: linear-gradient(135deg, #ffffff, #fff7ed); border-color: #fed7aa; box-shadow: 0 4px 0 #fed7aa;">
                    <div style="color: var(--accent-orange); display: flex; justify-content: center; margin-bottom: 4px;">
                    ...
                    <div style="font-size: 9px; font-weight: 800; color: #9a3412; text-transform: uppercase;">Streak</div>
                </div>

                <!-- Gems Card -->
                <div class="card-3d" style="padding: 14px 10px; text-align: center; background: linear-gradient(135deg, #ffffff, #eff6ff); border-color: #bfdbfe; box-shadow: 0 4px 0 #bfdbfe;">
                    <div style="color: var(--primary-blue); display: flex; justify-content: center; margin-bottom: 4px;">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M6 3h12l4 6-10 13L2 9Z"></path><path d="M11 3 8 9l4 13 4-13-3-6"></path><path d="M2 9h20"></path></svg>
                    </div>
                    <div style="font-size: 17px; font-weight: 900; color: var(--primary-blue);">{{ $user->gems }}</div>
                    <div style="font-size: 9px; font-weight: 800; color: #1e40af; text-transform: uppercase;">Gems</div>
                </div>

                <!-- Nyawa Card -->
                <div class="card-3d" style="padding: 14px 10px; text-align: center; background: linear-gradient(135deg, #ffffff, #fef2f2); border-color: #fecaca; box-shadow: 0 4px 0 #fecaca;">
                    <div style="color: var(--accent-red); display: flex; justify-content: center; margin-bottom: 4px;">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"></path></svg>
                    </div>
                    <div style="font-size: 17px; font-weight: 900; color: var(--accent-red);">{{ $user->hearts }}/5</div>
                    <div style="font-size: 9px; font-weight: 800; color: #991b1b; text-transform: uppercase;">Nyawa</div>
                </div>
            </div>

            <!-- Hearts Refill Card (If not full) -->
            @if ($user->hearts < 5)
                <div class="card-3d" style="padding: 20px; border-color: #fca5a5; background: linear-gradient(135deg, #fff, #fef2f2); box-shadow: 0 4px 0 #fca5a5;">
            ...
            <div class="card-3d" style="padding: 20px;">
                ...
                <div style="margin-bottom: 14px;">
                    ...
                    <div style="height: 10px; background: #e2e8f0; border-radius: 9999px; overflow: hidden; padding: 1px;">
                        <div style="height: 100%; width: {{ min(100, ($user->xp / 30) * 100) }}%; background: linear-gradient(90deg, #2563eb, #06b6d4); border-radius: 9999px;"></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div style="height: 10px; background: #e2e8f0; border-radius: 9999px; overflow: hidden; padding: 1px;">
                        <div style="height: 100%; width: 100%; background: linear-gradient(90deg, #10b981, #34d399); border-radius: 9999px;"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-3d" style="padding: 20px;">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <div style="width: 28px; height: 28px; border-radius: 8px; background: #fef3c7; color: #d97706; display: flex; align-items: center; justify-content: center;">
                ...
                @foreach ($topStudents as $rank => $student)
                    <div style="display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: {{ $loop->last ? 'none' : '1px solid #f1f5f9' }};">
                        <div style="font-size: 13px; font-weight: 900; width: 22px; height: 22px; border-radius: 6px; background: {{ $rank < 3 ? ($rank == 0 ? '#fef3c7' : ($rank == 1 ? '#f1f5f9' : '#fff7ed')) : '#f8fafc' }}; color: {{ $rankColors[$rank] ?? '#64748b' }}; display: flex; align-items: center; justify-content: center;">
                            {{ $rank + 1 }}
                        </div>
